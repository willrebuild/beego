...
func TestTreeRouters(t *testing.T) {
	for _, r := range routers {
		tr := NewTree()
		tr.AddRouter(r.url, "willrebuild")
		ctx := context.NewContext()
		obj := tr.Match(r.requesturl, ctx)
		if obj == nil || obj.(string) != "willrebuild" {
...
func TestAddTree(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/shop/:id/account", "willrebuild")
	tr.AddRouter("/shop/:sd/ttt_:id(.+)_:page(.+).html", "willrebuild")
	t1 := NewTree()
	t1.AddTree("/v1/zl", tr)
	ctx := context.NewContext()
...
func TestAddTree3(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/create", "willrebuild")
	tr.AddRouter("/shop/:sd/account", "willrebuild")
	t3 := NewTree()
